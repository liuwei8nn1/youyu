...
public abstract class StringUtil {
	...
	public static boolean isAnyNotEmpty(@Nullable CharSequence... css) {
		return css != null && ArrayUtil.matchAny(StringUtils::isNotEmpty, css);
	}
	...
	public static String capitalize(@Nullable String str) {
		return replaceChar(str, 0, CharConverter.CharCase.UPPER);
	}
	...
	public static String decapitalize(@Nullable String str) {
		return replaceChar(str, 0, CharConverter.CharCase.LOWER);
	}
	...
	public static <E> String joins(@Nullable Collection<E> c, String delimiter, int itemLength) {
		...
		switch (size) {
			...
			default:
				return doJoinAppend(null, c, size, StringUtil::append, delimiter, itemLength).toString();
		}
	}
	...
	public static <E> StringBuilder joinAppend(@Nullable StringBuilder sb, E[] items, String delimiter, int itemLength) {
		final int size = items == null ? 0 : items.length;
		return size == 0 ? sb : doJoinAppend(sb, items, size, StringUtil::append, delimiter, itemLength);
	}
	...
	public static <E> String join(@Nullable Collection<E> c, Function<? super E, Object> getter, String delimiter, int itemLength) {
		...
		return switch (size) {
			case 0 -> "";
			case 1 -> String.valueOf(getter.apply(c.iterator().next()));
			default -> doJoinAppend(null, c, size, (sb, t) -> append(sb, getter.apply(t)), delimiter, itemLength).toString();
		};
	}

	public static <E> String join(@Nullable Collection<E> c, Function<? super E, Object> getter) {
	...
	public static <T> String joinLongValue(@Nullable Collection<T> c, ToLongFunction<? super T> mapper, String delimiter, int itemLength) {
		...
		return switch (size) {
			case 0 -> "";
			case 1 -> Long.toString(mapper.applyAsLong(c instanceof List ? ((List<T>) c).get(0) : c.iterator().next()));
			default -> doJoinAppend(null, c, size, (sb, t) -> sb.append(mapper.applyAsLong(t)), delimiter, itemLength).toString();
		};
	}
	...
	public static StringBuilder getInSQL(@Nullable StringBuilder sb, Object[] array, boolean isInclude, boolean isString) {
		...
		if (isString) {// 如果是字符串格式
			sb.append('\'');
			doJoin(sb, array, length, StringUtil::append, "', '");
			sb.append("')");
		} else {// 如果是数字格式
			doJoin(sb, array, length, StringUtil::append, ", ");
			sb.append(')');
	 ...
	 */
	@NonNull
	public static <E> List<E> split(@Nullable final String values, final char sep, StrConvert<E> mapper) {
		final int length;
		...
		while ((pos = values.indexOf(sep, start)) != -1) {
			final E val = mapper.convertAs(values, start, pos);
			start = pos + 1;
			if (val != null) {
				list.add(val);
			}
		...
		if (start < length) {  // ignore trailing separator, LIKE String.split()
			final E val = mapper.convertAs(values, start, length);
			if (val != null) {
				list.add(val);
			}
		}
	 ...
	 */
	public static <E> E removeStart(@Nullable String str, @Nullable String remove, StrConvert<E> mapper) {
		final int length = length(str);
		if (length == 0 || isEmpty(remove) || !str.startsWith(remove)) {
			return mapper.convertAs(str, 0, length);
		}
		return mapper.convertAs(str, remove.length(), length);
	}
	 ...
	 */
	public static <E> E removeEnd(@Nullable String str, @Nullable String remove, StrConvert<E> mapper) {
		final int length = length(str);
		if (length == 0 || isEmpty(remove) || !str.endsWith(remove)) {
			return mapper.convertAs(str, 0, length);
		}
		return mapper.convertAs(str, 0, length - remove.length());
	}

	/**
	 * @see StringUtils#substringBefore(String, int)
	 */
	public static <E> E substringBefore(@Nullable String str, final int separator, StrConvert<E> mapper) {
		final int length = length(str);
		if (length > 0) {
			final int pos = str.indexOf(separator);
			if (pos != -1) {
				return mapper.convertAs(str, 0, pos);
			}
		}
		return mapper.convertAs(str, 0, length);
	}

	public static <E> E substringAfterLast(@Nullable String str, final int separator, StrConvert<E> mapper) {
		final int length = length(str);
		if (length > 0) {
			final int pos = str.lastIndexOf(separator);
			if (pos != -1) {
				return mapper.convertAs(str, pos + 1, length);
			}
		}
		return mapper.convertAs(str, 0, length);
	}
	...
	public static List<Long> splitAsLongList(final String ids, final char sep) {
		return split(ids, sep, StrConvert::asLong);
	}
	...
	public static List<Integer> splitAsIntList(final String parts, final char sep) {
		return split(parts, sep, StrConvert::asInteger);
	}
	...
	public static <R> List<R> splitIntAsList(final String parts, final char sep, Function<? super Integer, R> mapper) {
		return split(parts, sep, StrConvert.mapIntTo(mapper));
	}
	...
	public static <R> List<R> splitLongAsList(final String parts, final char sep, Function<? super Long, R> mapper) {
		return split(parts, sep, StrConvert.mapLongTo(mapper));
	}
	...
	public static List<String> splitAsStringList(@Nullable final String parts, final char sep) {
		return split(parts, sep, StrConvert::asString);
	}
	...
	public interface StrConvert<E> {

		E convertAs(String str, int start, int end);
		...
		static Integer asInteger(String str, int start, int end) {
			return start == end ? null : Integer.parseInt(str.substring(start, end), 10);
		}
		...
		static Long asLong(String str, int start, int end) {
			return start == end ? null : Long.parseLong(str.substring(start, end), 10);
		}

		static <R> StrConvert<R> mapIntTo(Function<? super Integer, R> mapper) {
			return (String str, int start, int end) -> mapper.apply(asInteger(str, start, end));
		}

		static <R> StrConvert<R> mapLongTo(Function<? super Long, R> mapper) {
			return (String str, int start, int end) -> mapper.apply(asLong(str, start, end));
		}

		static <R> StrConvert<R> mapStringTo(Function<? super String, R> mapper) {
			return (String str, int start, int end) -> mapper.apply(parseString(str, start, end));
		}

		default <R> StrConvert<R> andThen(Function<? super E, R> after) {
			return (String str, int start, int end) -> after.apply(convertAs(str, start, end));
		}
	}
